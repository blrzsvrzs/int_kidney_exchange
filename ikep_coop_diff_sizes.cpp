...
void changing_nodes(ListGraph::NodeMap<bool>& active_nodes, vector<bool>& leaving, vector<unsigned short int>& no_of_active_nodes, unsigned short int& N, vector<unsigned short int>& Vp_start, vector<unsigned short int>& node_arrives, unsigned short int& Q, vector<ListGraph::Node>& c, vector<unsigned short int>& s);
void lex_min_without_credits(vector<unsigned short int>& node_arrives, ListGraph& g, vector<bool>& leaving, ListGraph::NodeMap<bool>& active_nodes, vector<ListGraph::Node>& c, vector<unsigned short int>& s_LMWC, vector<unsigned short int>& no_of_active_nodes, unsigned short int& N, vector<unsigned short int>& Vp_start, unsigned short int& periods, bool& dispy, vector<unsigned short int>& s, unsigned short int& Q, vector<unsigned short int>& v, unsigned int& S, bool& target_nucl, double& min_satisfaction, vector<double>& target, vector<double>& target_LMWC, vector<double>& credit, double& prec, vector<double>& y, vector<bool>& pos, vector<unsigned short int>& w, unsigned short int& p, double& opt, vector<unsigned short int>& lb, vector<unsigned short int>& ub, ListGraph::EdgeMap<unsigned short int>& edge_card_weight, bool& empty_core);
void no_lex_min(vector<unsigned short int>& node_arrives, ListGraph& g, vector<bool>& leaving, ListGraph::NodeMap<bool>& active_nodes, vector<ListGraph::Node>& c, vector<unsigned short int>& s_NLM, vector<unsigned short int>& no_of_active_nodes, unsigned short int& N, vector<unsigned short int>& Vp_start, unsigned short int& periods, bool& dispy, vector<unsigned short int>& s, bool& target_nucl, vector<double>& target, vector<unsigned short int>& v, unsigned int& S, double& prec, double& min_satisfaction, vector<double>& target_NLM, bool& empty_core, vector<unsigned short int>& s_noncoop);
void ideal_matching(unsigned int& S, ListGraph& g, unsigned short int& N, vector<unsigned short int>& Vp_start, vector<ListGraph::Node>& c, bool& target_nucl, bool& disp, vector<double>& target, double& prec, double& min_satisfaction, bool& empty_core, vector<double>& y, vector<bool>& pos, vector<unsigned short int>& w, unsigned short int& p, vector<unsigned short int>& lb, vector<unsigned short int>& ub, double& opt, unsigned short int& no_of_nodes, unsigned short int& ideal_sum, vector<unsigned short int>& s, vector<unsigned short int>& Vp);
void country_sizes(unsigned short int& N, vector<unsigned short int>& Vp, vector<unsigned short int>& Vp_start, bool& dispy);
...
int main() {
	...
	unsigned short int graph_size = 1000; // the varying country size version currently only handles 1000 node graphs
	bool target_nucl = false;
	bool disp = false;
	...
	country_sizes(N, Vp, Vp_start, disp);
	...
	ideal_matching(S, g_ideal, N, Vp_start, c, target_nucl, disp, target_ideal, prec, min_satisfaction, empty_core, y, pos, w, p, lb, ub, opt, no_of_nodes, ideal_sum, s_ideal, Vp);
	t1 = cpuTime();
	...
	while (Q < periods) {
		...
		lex_min_matching(N, v[S], S, target, s, no_of_active_nodes, g, edge_card_weight, c, prec, disp, y, pos, w, p, lb, ub, opt, active_nodes, leaving, credit, Vp_start);
		t1 = cpuTime();
		matching_time += t1 - t0;
		t0 = t1;
		for (unsigned short int i = 0; i < N; i++)
		...
		for (unsigned short int i = 0; i < N; i++)
			s_cumm[i] += s[i];
		Q++;
		changing_nodes(active_nodes, leaving, no_of_active_nodes, N, Vp_start, node_arrives, Q, c, s);
		if (disp)
			cin.get();
	}
	t0 = cpuTime();
	...
	no_lex_min(node_arrives, g, leaving, active_nodes, c, s_NLM, no_of_active_nodes, N, Vp_start, periods, disp, s, target_nucl, target, v, S, prec, min_satisfaction, target_NLM, empty_core, s_NC);
	t1 = cpuTime();
	...
	lex_min_without_credits(node_arrives, g, leaving, active_nodes, c, s_LMWC, no_of_active_nodes, N, Vp_start, periods, disp, s, Q, v, S, target_nucl, min_satisfaction, target, target_LMWC, credit, prec, y, pos, w, p, opt, lb, ub, edge_card_weight, empty_core);
	t1 = cpuTime();
...
void country_sizes(unsigned short int& N, vector<unsigned short int>& Vp, vector<unsigned short int>& Vp_start, bool& dispy) {
	if (N == 5) {
		Vp[0] = 100; Vp[1] = 200; Vp[2] = 200; Vp[3] = 200; Vp[4] = 300;
	}
	else {
		if (N == 7) {
			Vp[0] = 68; Vp[1] = 68; Vp[2] = 136; Vp[3] = 136; Vp[4] = 136; Vp[5] = 204; Vp[6] = 204;
		}
		else {
			if (N == 9) {
				Vp[0] = 52; Vp[1] = 52; Vp[2] = 52; Vp[3] = 104; Vp[4] = 104; Vp[5] = 104; Vp[6] = 156; Vp[7] = 156; Vp[8] = 156;
			}
			else {
				for (unsigned short int i = 0; i < N / 2; i++) {
					Vp[i] = 4 * (unsigned short int)(1000 / (6 * N));
					Vp[N - i] = 2 * Vp[i];
				}
				Vp[N / 2] = Vp[N - 1];
			}
...
void lex_min_without_credits(vector<unsigned short int> &node_arrives, ListGraph &g, vector<bool> &leaving, ListGraph::NodeMap<bool> &active_nodes, vector<ListGraph::Node> &c, vector<unsigned short int> &s_LMWC, vector<unsigned short int> &no_of_active_nodes, unsigned short int &N, vector<unsigned short int> &Vp_start, unsigned short int &periods, bool &dispy, vector<unsigned short int> &s, unsigned short int &Q, vector<unsigned short int> &v, unsigned int &S, bool &target_nucl, double &min_satisfaction, vector<double> &target, vector<double> &target_LMWC, vector<double> &credit, double &prec, vector<double> &y, vector<bool> &pos, vector<unsigned short int> &w, unsigned short int &p, double &opt, vector<unsigned short int> &lb, vector<unsigned short int> &ub, ListGraph::EdgeMap<unsigned short int> &edge_card_weight, bool &empty_core) {
	...
	while (Q < periods) {
		...
		lex_min_matching(N, v[S], S, target, s, no_of_active_nodes, g, edge_card_weight, c, prec, dispy, y, pos, w, p, lb, ub, opt, active_nodes, leaving, credit, Vp_start);
		for (unsigned short int i = 0; i < N; i++)
			s_LMWC[i] += s[i];
		Q++;
		changing_nodes(active_nodes, leaving, no_of_active_nodes, N, Vp_start, node_arrives, Q, c, s);
...
void lex_min_matching(unsigned short int &N, unsigned short int &grandcoal, unsigned int &S, vector<double> &target, vector<unsigned short int> &s, vector<unsigned short int> &no_of_active_nodes, ListGraph &g, ListGraph::EdgeMap<unsigned short int> &edge_card_weight, vector<ListGraph::Node> &c, double &prec, bool &dispy, vector<double> &y, vector<bool> &pos, vector<unsigned short int> &w, unsigned short int &p, vector<unsigned short int> &lb, vector<unsigned short int> &ub, double &opt, ListGraph::NodeMap<bool> &active_nodes, vector<bool> &leaving, vector<double> &credit, vector<unsigned short int> &Vp_start) {
	...
	while (p < N) {
		...
		new_matching(lb, ub, g, edge_card_weight, w, p, y, c, opt, s, grandcoal, prec, dispy, target, no_of_active_nodes, N, active_nodes, pos, leaving, credit, Vp_start);
	}
	return;
}
