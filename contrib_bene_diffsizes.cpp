...
int main() {
	...
	ideal_matching(g_ideal, N, Vp_start, c, target_benefit, disp, target_ideal, prec, y, pos, w, p, lb, ub, opt, no_of_nodes, ideal_sum, s_ideal, Vp);
	t1 = cpuTime();
	...
	while (Q < periods) {
		...
		lex_min_matching(N, v[N], target, s, no_of_active_nodes, g, edge_card_weight, c, prec, disp, y, pos, w, p, lb, ub, opt, active_nodes, leaving, credit, Vp_start);
		t1 = cpuTime();
		matching_time += t1 - t0;
		t0 = t1;
		for (unsigned short int i = 0; i < N; i++)
	...
	no_lex_min(node_arrives, g, leaving, active_nodes, c, s_NLM, no_of_active_nodes, N, Vp_start, periods, disp, s, target_benefit, target, v, v_impu, prec, target_NLM, s_NC);
	t1 = cpuTime();
	...
	lex_min_without_credits(node_arrives, g, leaving, active_nodes, c, s_LMWC, no_of_active_nodes, N, Vp_start, periods, disp, s, Q, v, v_impu, target_benefit, target, target_LMWC, credit, prec, y, pos, w, p, opt, lb, ub, edge_card_weight);
	t1 = cpuTime();
...
void lex_min_without_credits(vector<unsigned short int> &node_arrives, ListGraph &g, vector<bool> &leaving, ListGraph::NodeMap<bool> &active_nodes, vector<ListGraph::Node> &c, vector<unsigned short int> &s_LMWC, vector<unsigned short int> &no_of_active_nodes, unsigned short int &N, vector<unsigned short int> &Vp_start, unsigned short int &periods, bool &dispy, vector<unsigned short int> &s, unsigned short int &Q, vector<unsigned short int> &v, vector<unsigned short int>& v_impu, bool &target_benefit, vector<double> &target, vector<double> &target_LMWC, vector<double> &credit, double &prec, vector<double> &y, vector<bool> &pos, vector<unsigned short int> &w, unsigned short int &p, double &opt, vector<unsigned short int> &lb, vector<unsigned short int> &ub, ListGraph::EdgeMap<unsigned short int> &edge_card_weight) {
	...
	while (Q < periods) {
		...
		lex_min_matching(N, v[N], target, s, no_of_active_nodes, g, edge_card_weight, c, prec, dispy, y, pos, w, p, lb, ub, opt, active_nodes, leaving, credit, Vp_start);
		for (unsigned short int i = 0; i < N; i++)
			s_LMWC[i] += s[i];
		Q++;
		changing_nodes(active_nodes, leaving, no_of_active_nodes, N, Vp_start, node_arrives, Q, c, s);
