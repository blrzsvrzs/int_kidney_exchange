...
*             benefit and contribution value allocations with varying country sizes
*
...
int main() {
	...
	srand(seed);
	ofstream res;
	initial_pairs(Vp_start, N, active_nodes, c);
	...
	t0 = cpuTime();
	ideal_matching(S, g_ideal, N, Vp_start, c, target_omega, disp, dispy, prec, y, pos, w, p, lb, ub, opt, no_of_nodes, s_ideal, t0, d1, init_alloc_ideal, s_ideal_d1, ideal_time, ideal_d1_time, v_impu, Vp);
	ideal_time += init_time + read_time + rand_time + graph_time;
	ideal_d1_time += init_time + read_time + rand_time + graph_time;
	cout << "ideal done... ";
	t0 = cpuTime();

	LMC(Q, periods, dispy, N, no_of_active_nodes, g, v, s, c, disp, S, leaving, active_nodes, Vp_start, t1, game_time, target_omega, t0, target, prec, target_time, init_alloc_LMC, I1, I11, I2, edge_card_weight, y, pos, w, p, lb, ub, opt, d1, credit, matching_time, actual_alloc_LMC, node_arrives, v_impu);
	...
	t0 = cpuTime();
	arbitrary_matching(node_arrives, g, leaving, active_nodes, c, disp, no_of_active_nodes, N, Vp_start, periods, dispy, s, target_omega, target, v, S, prec, t0, actual_alloc_NC, actual_alloc_rand, init_alloc_rand, v_impu);
	t1 = cpuTime();
	double arbitrary_time = t1 - t0 + init_time + read_time + rand_time + graph_time;
	cout << "arbitrary matching done... ";

	t0 = cpuTime();
	lex_min_without_credits(node_arrives, g, leaving, active_nodes, c, disp, no_of_active_nodes, N, Vp_start, periods, dispy, s, Q, v, S, target_omega, target, credit, prec, y, pos, w, p, opt, lb, ub, edge_card_weight, t0, d1, I, init_alloc_LM, actual_alloc_LM, v_impu);
	t1 = cpuTime();
	...
	if (second) {
		...
		I = 0;
		d1 = true;
		LMC(Q, periods, dispy, N, no_of_active_nodes, g, v, s, c, disp, S, leaving, active_nodes, Vp_start, t1, tmp, target_omega, tmp, target, prec, tmp, init_alloc_d1C, I, I, I, edge_card_weight, y, pos, w, p, lb, ub, opt, d1, credit, tmp, actual_alloc_d1C, node_arrives, v_impu);
		t1 = cpuTime();
		...
		d1 = true;
		lex_min_without_credits(node_arrives, g, leaving, active_nodes, c, disp, no_of_active_nodes, N, Vp_start, periods, dispy, s, Q, v, S, target_omega, target, credit, prec, y, pos, w, p, opt, lb, ub, edge_card_weight, t0, d1, I, init_alloc_d1, actual_alloc_d1, v_impu);
		t1 = cpuTime();
...
void LMC(unsigned short int& Q, unsigned short int& periods, bool& dispy, unsigned short int& N, vector<unsigned short int>& no_of_active_nodes, ListGraph& g, vector<unsigned short int>& v, vector<unsigned short int>& s, vector<ListGraph::Node>& c, bool& disp, unsigned int& S, vector<bool>& leaving, ListGraph::NodeMap<bool>& active_nodes, vector<unsigned short int>& Vp_start, double& t1, double& game_time, bool& target_omega, double& t0, vector<double>& target, double& prec, double& target_time, vector<vector<double>>& init_alloc, unsigned short int& I1, unsigned short int& I11, unsigned short int& I2, ListGraph::EdgeMap<unsigned short int>& edge_card_weight, vector<double>& y, vector<bool>& pos, vector<unsigned short int>& w, unsigned short int& p, vector<unsigned short int>& lb, vector<unsigned short int>& ub, double& opt, bool& d1, vector<double>& credit, double& matching_time, vector<vector<unsigned short int>>& actual_alloc, vector<unsigned short int>& node_arrives, vector<unsigned short int>& v_impu) {
	while (Q < periods) {
		if (disp) {
		...
		coop_game(g, v, v_impu, s, c, disp, Vp_start, N, active_nodes, leaving);
		t1 = cpuTime();
		game_time += t1 - t0;
		t0 = cpuTime();
		double suma = 0;
		...
		init_alloc[Q] = target;
		t0 = cpuTime();
		lex_min_matching(N, v[N], S, target, s, no_of_active_nodes, I1, I11, I2, g, edge_card_weight, c, prec, disp, y, pos, w, p, lb, ub, opt, active_nodes, leaving, credit, disp, Vp_start, d1);
		t1 = cpuTime();
		matching_time += t1 - t0;
		actual_alloc[Q] = s;
		t0 = cpuTime();
		...
		Q++;
		changing_nodes(active_nodes, leaving, no_of_active_nodes, N, Vp_start, node_arrives, Q, disp, c, s);
		if (disp)
			cin.get();
	}
	return;
...
void ideal_matching(unsigned int& S, ListGraph& g, unsigned short int& N, vector<unsigned short int>& Vp_start, vector<ListGraph::Node>& c, bool& target_omega, bool& disp, bool& dispy, double& prec, vector<double>& y, vector<bool>& pos, vector<unsigned short int>& w, unsigned short int& p, vector<unsigned short int>& lb, vector<unsigned short int>& ub, double& opt, unsigned short int& no_of_nodes, vector<unsigned short int>& s_ideal, double& t0, bool& d1, vector<double>& target_ideal, vector<unsigned short int>& s_ideal_d1, double& ideal_time, double& ideal_d1_time, vector<unsigned short int>& v_impu, vector<unsigned short int>& Vp) {
	...
	t0 = cpuTime();
	lex_min_matching(N, v[N], S, target_ideal, s_ideal, no_of_active_nodes, I, I, I, g, edge_card_weight, c, prec, dispy, y, pos, w, p, lb, ub, opt, active_nodes, leaving, credit, disp, Vp_start, d1);
	ideal_time += cpuTime() - t0;
	t0 = cpuTime();
	d1 = true;
	lex_min_matching(N, v[N], S, target_ideal, s_ideal_d1, no_of_active_nodes, I, I, I, g, edge_card_weight, c, prec, dispy, y, pos, w, p, lb, ub, opt, active_nodes, leaving, credit, disp, Vp_start, d1);
...
void lex_min_without_credits(vector<unsigned short int>& node_arrives, ListGraph& g, vector<bool>& leaving, ListGraph::NodeMap<bool>& active_nodes, vector<ListGraph::Node>& c, bool& disp, vector<unsigned short int>& no_of_active_nodes, unsigned short int& N, vector<unsigned short int>& Vp_start, unsigned short int& periods, bool& dispy, vector<unsigned short int>& s, unsigned short int& Q, vector<unsigned short int>& v, unsigned int& S, bool& target_omega, vector<double>& target, vector<double>& credit, double& prec, vector<double>& y, vector<bool>& pos, vector<unsigned short int>& w, unsigned short int& p, double& opt, vector<unsigned short int>& lb, vector<unsigned short int>& ub, ListGraph::EdgeMap<unsigned short int>& edge_card_weight, double& t0, bool& d1, unsigned short int& I, vector<vector<double>>& init_alloc, vector<vector<unsigned short int>>& actual_alloc, vector<unsigned short int>& v_impu) {
	...
	while (Q < periods) {
		...
		// cooperative game and target
		coop_game(g, v, v_impu, s, c, dispy, Vp_start, N, active_nodes, leaving);
		double suma = 0;
		double sumimpu = 0;
		if (target_omega) {
			for (unsigned short int i = 0; i < N; i++) {
		...
		lex_min_matching(N, v[N], S, target, s, no_of_active_nodes, I, I, I, g, edge_card_weight, c, prec, dispy, y, pos, w, p, lb, ub, opt, active_nodes, leaving, credit, disp, Vp_start, d1);
		actual_alloc[Q] = s;
		Q++;
		changing_nodes(active_nodes, leaving, no_of_active_nodes, N, Vp_start, node_arrives, Q, disp, c, s);
		if (dispy)
...
void lex_min_matching(unsigned short int& N, unsigned short int& grandcoal, unsigned int& S, vector<double>& target, vector<unsigned short int>& s, vector<unsigned short int>& no_of_active_nodes, unsigned short int& I1, unsigned short int& I11, unsigned short int& I2, ListGraph& g, ListGraph::EdgeMap<unsigned short int>& edge_card_weight, vector<ListGraph::Node>& c, double& prec, bool& dispy, vector<double>& y, vector<bool>& pos, vector<unsigned short int>& w, unsigned short int& p, vector<unsigned short int>& lb, vector<unsigned short int>& ub, double& opt, ListGraph::NodeMap<bool>& active_nodes, vector<bool>& leaving, vector<double>& credit, bool& disp, vector<unsigned short int>& Vp_start, bool& d1) {
	...
	while (p < N) {
		...
		new_matching(lb, ub, g, edge_card_weight, w, p, y, c, opt, s, grandcoal, prec, dispy, target, no_of_active_nodes, N, active_nodes, pos, leaving, credit, Vp_start, disp, I1, I11, I2);
		if (p > 0 && d1)
			break;
	}
	return;
	...
	if ((lb[w[p]] == ub[w[p]]) && (s[w[p]] == ub[w[p]]) && (lb[w[p]] == s[w[p]])) {
		...
		for (unsigned short int i = 0; i < N; i++) {
			for (unsigned short int j = 0; j < no_of_active_nodes[w[i]] - ub[w[i]]; j++) {
				...
				for (unsigned short int k = Vp_start[w[i]]; k < Vp_start[w[i] + 1]; k++) {
					if (active_nodes[c[k]]) {
						ListGraph::Edge e = g.addEdge(Bp[w[i]][j], c[k]);
			...
			for (unsigned short int j = 0; j < ub[w[i]] - lb[w[i]]; j++) {
				...
				for (unsigned short int k = Vp_start[w[i]]; k < Vp_start[w[i] + 1]; k++) {
					if (active_nodes[c[k]]) {
						ListGraph::Edge e = g.addEdge(Ap[w[i]][j], c[k]);
						edge_card_weight[e] = 0;
					}
				}
		...
		if (abs(opt - max_match) < prec) {
			...
			for (unsigned short int i = 0; i < Vp_start[N]; i++) {
				if (active_nodes[c[i]]) {
					if (edge_card_weight[max_weight.matching(c[i])] == 1) {
						for (unsigned short int j = 0; j < N; j++) {
							if (Vp_start[j] <= i && i < Vp_start[j + 1]) {
								t[j]++;
								break;
							}
						}
	...
	for (unsigned short int i = 0; i < N; i++) {
		...
		for (unsigned short int j = Vp_start[i]; j < Vp_start[i + 1]; j++) {
			if (leaving[j]) {
				...
				leaving[j] = false;
				if (disp)
					cout << "Node " << j << " is leaving at " << Q << " becaused its matched" << endl;
			}
			else {
				if (active_nodes[c[j]] && node_arrives[j] == Q - 4) {
					...
					no_of_active_nodes[i]--;
					if (disp)
						cout << "Node " << j << " is leaving at " << Q << " because spent 1 year (unmatched) in the program" << endl;
				}
			}
			if (node_arrives[j] == Q) {
				active_nodes[c[j]] = true;
				no_of_active_nodes[i]++;
				if (disp)
					cout << "Node " << j << " is entering at " << Q << endl;
			}
		}
	}
	return;
