...
*             Shapley value and nucleolus allocations with varying country sizes
*
...
void coop_game(ListGraph& g, vector<unsigned short int>& v, vector<unsigned short int>& s, vector<ListGraph::Node>& c, bool& dispy, unsigned int& S, vector<unsigned short int>& Vp_start, unsigned short int& N, ListGraph::NodeMap<bool>& active_nodes, vector<bool>& leaving);
...
int main() {
	...
	t0 = cpuTime();
	ideal_matching(S, g_ideal, N, Vp_start, c, target_nucl, disp, dispy, prec, y, pos, w, p, lb, ub, opt, no_of_nodes, s_ideal, t0, d1, v, init_alloc_ideal, s_ideal_d1, ideal_time, ideal_d1_time, Vp);
	ideal_time += init_time + read_time + rand_time + graph_time;
	ideal_d1_time += init_time + read_time + rand_time + graph_time;
	cout << "ideal done... ";
	t0 = cpuTime();

	LMC(Q, periods, dispy, N, no_of_active_nodes, g, v, s, c, disp, S, leaving, active_nodes, Vp_start, t1, game_time, target_nucl, t0, target, prec, target_time, init_alloc_LMC, I1, I11, I2, edge_card_weight, y, pos, w, p, lb, ub, opt, d1, credit, matching_time, actual_alloc_LMC, node_arrives);
	...
	d1 = true;
	LMC(Q, periods, dispy, N, no_of_active_nodes, g, v, s, c, disp, S, leaving, active_nodes, Vp_start, t1, tmp, target_nucl, tmp, target, prec, tmp, init_alloc_d1C, I, I, I, edge_card_weight, y, pos, w, p, lb, ub, opt, d1, credit, tmp, actual_alloc_d1C, node_arrives);
	t1 = cpuTime();
	...
	t0 = cpuTime();
	arbitrary_matching(node_arrives, g, leaving, active_nodes, c, disp, no_of_active_nodes, N, Vp_start, periods, dispy, s, target_nucl, target, v, S, prec, t0, actual_alloc_NC, actual_alloc_rand, init_alloc_rand);
	t1 = cpuTime();
	double arbitrary_time = t1 - t0 + init_time + read_time + rand_time + graph_time;
	cout << "arbitrary matching done... ";

	t0 = cpuTime();
	lex_min_without_credits(node_arrives, g, leaving, active_nodes, c, disp, no_of_active_nodes, N, Vp_start, periods, dispy, s, Q, v, S, target_nucl, target, credit, prec, y, pos, w, p, opt, lb, ub, edge_card_weight, t0, d1, I, init_alloc_LM, actual_alloc_LM);
	t1 = cpuTime();
	...
	d1 = true;
	lex_min_without_credits(node_arrives, g, leaving, active_nodes, c, disp, no_of_active_nodes, N, Vp_start, periods, dispy, s, Q, v, S, target_nucl, target, credit, prec, y, pos, w, p, opt, lb, ub, edge_card_weight, t0, d1, I, init_alloc_d1, actual_alloc_d1);
	t1 = cpuTime();
...
void lex_min_without_credits(vector<unsigned short int>& node_arrives, ListGraph& g, vector<bool>& leaving, ListGraph::NodeMap<bool>& active_nodes, vector<ListGraph::Node>& c, bool& disp, vector<unsigned short int>& no_of_active_nodes, unsigned short int& N, vector<unsigned short int>& Vp_start, unsigned short int& periods, bool& dispy, vector<unsigned short int>& s, unsigned short int& Q, vector<unsigned short int>& v, unsigned int& S, bool& target_nucl, vector<double>& target, vector<double>& credit, double& prec, vector<double>& y, vector<bool>& pos, vector<unsigned short int>& w, unsigned short int& p, double& opt, vector<unsigned short int>& lb, vector<unsigned short int>& ub, ListGraph::EdgeMap<unsigned short int>& edge_card_weight, double& t0, bool& d1, unsigned short int& I, vector<vector<double>>& init_alloc, vector<vector<unsigned short int>>& actual_alloc) {
	...
	while (Q < periods) {
		...
		coop_game(g, v, s, c, dispy, S, Vp_start, N, active_nodes, leaving);
		if (target_nucl)
		...
		lex_min_matching(N, v[S], S, target, s, no_of_active_nodes, I, I, I, g, edge_card_weight, c, prec, dispy, y, pos, w, p, lb, ub, opt, active_nodes, leaving, credit, disp, Vp_start, d1);
		actual_alloc[Q] = s;
		Q++;
...
void arbitrary_matching(vector<unsigned short int>& node_arrives, ListGraph& g, vector<bool>& leaving, ListGraph::NodeMap<bool>& active_nodes, vector<ListGraph::Node>& c, bool& disp, vector<unsigned short int>& no_of_active_nodes, unsigned short int& N, vector<unsigned short int>& Vp_start, unsigned short int& periods, bool& dispy, vector<unsigned short int>& s, bool& target_nucl, vector<double>& target, vector<unsigned short int>& v, unsigned int& S, double& prec, double& t0, vector<vector<unsigned short int>>& actual_alloc_NC, vector<vector<unsigned short int>>& actual_alloc, vector<vector<double>>& initial_alloc) {
	...
	for (unsigned short int Q = 0; Q < periods; Q++) {
		...
		coop_game(g, v, s, c, disp, S, Vp_start, N, active_nodes, leaving);
		if (target_nucl)
			nucl(disp, N, S, target, v, prec);
		else
			shapley(target, v, N, S);
...
void LMC(unsigned short int& Q, unsigned short int& periods, bool& dispy, unsigned short int& N, vector<unsigned short int>& no_of_active_nodes, ListGraph& g, vector<unsigned short int>& v, vector<unsigned short int>& s, vector<ListGraph::Node>& c, bool& disp, unsigned int& S, vector<bool>& leaving, ListGraph::NodeMap<bool>& active_nodes, vector<unsigned short int>& Vp_start, double& t1, double& game_time, bool& target_nucl, double& t0, vector<double>& target, double& prec, double& target_time, vector<vector<double>>& init_alloc, unsigned short int& I1, unsigned short int& I11, unsigned short int& I2, ListGraph::EdgeMap<unsigned short int>& edge_card_weight, vector<double>& y, vector<bool>& pos, vector<unsigned short int>& w, unsigned short int& p, vector<unsigned short int>& lb, vector<unsigned short int>& ub, double& opt, bool& d1, vector<double>& credit, double& matching_time, vector<vector<unsigned short int>>& actual_alloc, vector<unsigned short int>& node_arrives) {
	while (Q < periods) {
		...
		init_alloc[Q] = target;
		t0 = cpuTime();
		lex_min_matching(N, v[S], S, target, s, no_of_active_nodes, I1, I11, I2, g, edge_card_weight, c, prec, dispy, y, pos, w, p, lb, ub, opt, active_nodes, leaving, credit, disp, Vp_start, d1);
		t1 = cpuTime();
		matching_time += t1 - t0;
		actual_alloc[Q] = s;
		t0 = cpuTime();
...
void ideal_matching(unsigned int& S, ListGraph& g, unsigned short int& N, vector<unsigned short int>& Vp_start, vector<ListGraph::Node>& c, bool& target_nucl, bool& disp, bool& dispy, double& prec, vector<double>& y, vector<bool>& pos, vector<unsigned short int>& w, unsigned short int& p, vector<unsigned short int>& lb, vector<unsigned short int>& ub, double& opt, unsigned short int& no_of_nodes, vector<unsigned short int>& s_ideal, double& t0, bool& d1, vector<unsigned short int>& v_ideal, vector<double>& target_ideal, vector<unsigned short int>& s_ideal_d1, double& ideal_time, double& ideal_d1_time, vector<unsigned short int>& Vp) {
	...
	t0 = cpuTime();
	lex_min_matching(N, v_ideal[S], S, target_ideal, s_ideal, no_of_active_nodes, I, I, I, g, edge_card_weight, c, prec, dispy, y, pos, w, p, lb, ub, opt, active_nodes, leaving, credit, disp, Vp_start, d1);
	ideal_time += cpuTime() - t0;
	t0 = cpuTime();
	d1 = true;
	lex_min_matching(N, v_ideal[S], S, target_ideal, s_ideal_d1, no_of_active_nodes, I, I, I, g, edge_card_weight, c, prec, dispy, y, pos, w, p, lb, ub, opt, active_nodes, leaving, credit, disp, Vp_start, d1);
